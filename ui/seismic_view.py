...
class SeismicView(QMainWindow):
    def __init__(self):
        ...
        self.spin_step = QDoubleSpinBox(); self.spin_step.setDecimals(0); self.spin_step.setRange(1, 5000)
        self.spin_step.setValue(1); self.spin_step.setEnabled(False) 
        ...
        self.combo_domain = QComboBox(); self.combo_domain.addItems(["Time", "Depth"])
        self.sidebar_layout.addWidget(self.combo_domain)

        toggle_layout = QHBoxLayout()
        self.chk_flip_x = QCheckBox("Flip X"); self.chk_grid = QCheckBox("Grid"); self.chk_grid.setChecked(True)
        toggle_layout.addWidget(self.chk_flip_x); toggle_layout.addWidget(self.chk_grid)
        self.sidebar_layout.addLayout(toggle_layout)

        self.sidebar_layout.addWidget(QLabel("Colormap:"))
        self.combo_cmap = QComboBox(); all_cmaps = sorted(plt.colormaps()); self.combo_cmap.addItems(all_cmaps); self.combo_cmap.setCurrentText("seismic")
        self.sidebar_layout.addWidget(self.combo_cmap)
        
        self.sidebar_layout.addWidget(QLabel("Contrast (Percentile):"))
        self.spin_contrast = QDoubleSpinBox(); self.spin_contrast.setRange(50.0, 100.0); self.spin_contrast.setValue(99.0); self.spin_contrast.setSingleStep(0.1)
        ...
        dim_layout.addWidget(QLabel("W (in):")); self.spin_fig_width = QDoubleSpinBox(); self.spin_fig_width.setValue(10.0); dim_layout.addWidget(self.spin_fig_width)
        dim_layout.addWidget(QLabel("H (in):")); self.spin_fig_height = QDoubleSpinBox(); self.spin_fig_height.setValue(6.0); dim_layout.addWidget(self.spin_fig_height)
        export_layout.addLayout(dim_layout)
        
        self.btn_preview_ratio = QPushButton("Match Aspect Ratio"); export_layout.addWidget(self.btn_preview_ratio)
        self.btn_export = QPushButton("Export Figure"); self.btn_export.setStyleSheet("background-color: #ffcccc; font-weight: bold;"); export_layout.addWidget(self.btn_export)
        ...
        splitter.addWidget(self.sidebar); splitter.addWidget(self.plot_widget)
