...
class MainController:
    ...
    def load_data_internal(self, start, end, step, auto_fit=False):
        try:
            ...
            self.view.update_status("Reading Data..."); QApplication.processEvents()
            ...
            if auto_fit:
                ...
                self.view.spin_y_min.setValue(t_min); self.view.spin_y_max.setValue(t_max)
            
            self.update_contrast(); self.update_labels(); self.draw_horizons()
            self.is_programmatic_update = False
    ...
    def run_attribute(self, attr_type):
        ...
        if header == "Trace Index":
            start_trace = int(x_min); end_trace = int(x_max)
            ...
            try:
                ...
                start_trace = max(0, int(x_min)); end_trace = min(self.data_manager.n_traces, int(x_max))
        ...
        QApplication.processEvents()
        self.view.spin_x_min.setValue(x_min); self.view.spin_x_max.setValue(x_max)
        self.view.chk_manual_step.setChecked(True); self.view.spin_step.setValue(1)
        self.load_data_internal(start_trace, end_trace, step=1, auto_fit=False)

        # 4. Calculate
        if self.current_data is None: return
        self.view.update_status(f"Calculating {attr_type}..."); QApplication.processEvents()
        try:
            sr = self.data_manager.sample_rate
            if attr_type == "Envelope": self.current_data = SeismicProcessing.attribute_envelope(self.current_data)
            ...
            elif attr_type == "Frequency": self.current_data = SeismicProcessing.attribute_frequency(self.current_data, sr)
            elif attr_type == "Cosine Phase": self.current_data = SeismicProcessing.attribute_cosine_phase(self.current_data)
            elif attr_type == "RMS":
    ...
    def update_display_only(self):
        ...
        if hasattr(self, 'x_vals') and self.x_vals is not None and self.x_vals.size > 0:
            x_min = self.x_vals[0]; x_max = self.x_vals[-1]
        else:
            x_min = self.view.spin_x_min.value(); x_max = self.view.spin_x_max.value()
            
        if hasattr(self, 't_vals') and self.t_vals is not None:
            y_min = self.t_vals[0]; y_max = self.t_vals[-1]
        else:
    ...
    def show_amplitude_histogram(self):
        ...
        try:
            ...
            ax1.axvline(-clip_val, color='red', ls='--', lw=2); ax1.axvline(clip_val, color='red', ls='--', lw=2)
            ...
            ax2.axvline(-clip_val, color='red', ls='--', lw=2); ax2.axvline(clip_val, color='red', ls='--', lw=2)
            ax2.set_title(f'Dynamic Range ({within_clip/len(amps)*100:.2f}% mapped)')
            
            plt.tight_layout()
            plt.show()
        except Exception as e: QMessageBox.critical(self.view, "Histogram Error", str(e))
    ...
    def reset_view(self):
        ...
        header = self.view.combo_header.currentText(); x_min, x_max = 0, 0
        if header == "Trace Index": x_min, x_max = 0, self.data_manager.n_traces
        elif header == "Cumulative Distance" and self.full_cum_dist is not None: x_min, x_max = self.full_cum_dist[0], self.full_cum_dist[-1]
        elif self.active_header_map is not None: x_min, x_max = np.min(self.active_header_map), np.max(self.active_header_map)
        t_min, t_max = self.data_manager.time_axis[0], self.data_manager.time_axis[-1]
        self.view.spin_x_min.setValue(x_min); self.view.spin_x_max.setValue(x_max); self.view.spin_y_min.setValue(t_min); self.view.spin_y_max.setValue(t_max)
        self.view.chk_manual_step.setChecked(False); self.apply_changes()

    def calculate_distance(self, settings):
        self.view.update_status("Calculating Distance..."); QApplication.processEvents()
        try:
            ...
            if settings['use_header']:
                ...
                scaled_x = SeismicProcessing.apply_scalar(raw_x, scalars); scaled_y = SeismicProcessing.apply_scalar(raw_y, scalars)
            else: s = settings['manual_val']; scaled_x = raw_x * s; scaled_y = raw_y * s
            dist = SeismicProcessing.calculate_cumulative_distance(scaled_x, scaled_y)
            max_dist = dist[-1]
            if max_dist > 10000: dist = dist / 1000.0; self.dist_unit = "km"
            else: self.dist_unit = "m"
            self.full_cum_dist = dist
            if self.view.combo_header.findText("Cumulative Distance") == -1: self.view.combo_header.insertItem(1, "Cumulative Distance")
            self.view.combo_header.setCurrentText("Cumulative Distance"); self.on_header_changed()
            QMessageBox.information(self.view, "Success", f"Calculated distance. Length: {self.full_cum_dist[-1]:.2f} {self.dist_unit}")
        except Exception as e: QMessageBox.critical(self.view, "Error", f"Calculation failed:\n{str(e)}")
    ...
    def show_dist_tool(self):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted: settings = dlg.get_settings(); self.calculate_distance(settings)
    ...
    def apply_changes(self):
        if not self.data_manager: return
        self.view.update_status("Reloading data..."); QApplication.processEvents()
        ...
        if header != "Trace Index" and self.active_header_map is not None:
            if np.all(np.diff(self.active_header_map) >= 0):
                start_trace = np.searchsorted(self.active_header_map, target_x_min); end_trace = np.searchsorted(self.active_header_map, target_x_max)
            else:
    ...
    def handle_horizon_export(self, index):
        ...
        try:
            ...
            df = pd.DataFrame(); current_x_mode = self.view.combo_header.currentText()
            df[current_x_mode] = x_arr; df[self.view.combo_domain.currentText()] = y_arr
            trace_indices = np.zeros(len(x_arr), dtype=int)
            if current_x_mode == "Trace Index": trace_indices = np.round(x_arr).astype(int)
            elif self.active_header_map is not None:
                if np.all(np.diff(self.active_header_map) >= 0): trace_indices = np.searchsorted(self.active_header_map, x_arr); trace_indices = np.clip(trace_indices, 0, len(self.active_header_map)-1)
                else:
                    for i, val in enumerate(x_arr): trace_indices[i] = (np.abs(self.active_header_map - val)).argmin()
            for hdr in selected_headers: df[hdr] = self.data_manager.get_header_slice(hdr, 0, self.data_manager.n_traces, 1)[trace_indices]
    ...
    def on_plot_clicked(self, event):
        ...
        if self.view.plot_widget.plotItem.sceneBoundingRect().contains(pos):
            ...
            click_x = mousePoint.x(); click_y = mousePoint.y()
            trace_idx = int(click_x)
            header = self.view.combo_header.currentText()
            if header == "Trace Index": trace_idx = int(round(click_x))
            elif self.active_header_map is not None:
                if len(self.active_header_map) > 0: trace_idx = int((np.abs(self.active_header_map - click_x)).argmin())
            if event.button() == Qt.MouseButton.LeftButton: self.horizon_manager.add_point(trace_idx, click_y)
            elif event.button() == Qt.MouseButton.RightButton:
                view_range = self.view.plot_widget.viewRange(); y_height = abs(view_range[1][1] - view_range[1][0]); y_tol = y_height * 0.02
    ...
    def set_picking_mode(self, active, horizon_name): self.is_picking_mode = active; self.view.plot_widget.setCursor(Qt.CursorShape.CrossCursor if active else Qt.CursorShape.ArrowCursor)
    ...
    def setup_menu(self):
        ...
        file_menu = menu_bar.addMenu("File"); self.action_load_menu = file_menu.addAction("Load SEG-Y", self.load_file); file_menu.addAction("Export PDF/PNG", self.export_figure)
        ...
        self.act_env = QAction("Instantaneous Amplitude (Envelope)", self.view); self.act_env.triggered.connect(lambda: self.run_attribute("Envelope")); self.act_env.setEnabled(False); attr_menu.addAction(self.act_env)
        self.act_phase = QAction("Instantaneous Phase", self.view); self.act_phase.triggered.connect(lambda: self.run_attribute("Phase")); self.act_phase.setEnabled(False); attr_menu.addAction(self.act_phase)
        self.act_cos = QAction("Cosine of Phase", self.view); self.act_cos.triggered.connect(lambda: self.run_attribute("Cosine Phase")); self.act_cos.setEnabled(False); attr_menu.addAction(self.act_cos)
        self.act_freq = QAction("Instantaneous Frequency", self.view); self.act_freq.triggered.connect(lambda: self.run_attribute("Frequency")); self.act_freq.setEnabled(False); attr_menu.addAction(self.act_freq)
        attr_menu.addSeparator()
        self.act_rms = QAction("RMS Amplitude", self.view); self.act_rms.triggered.connect(lambda: self.run_attribute("RMS")); self.act_rms.setEnabled(False); attr_menu.addAction(self.act_rms)
        ...
        self.action_histogram = QAction("View Amplitude Histogram", self.view); self.action_histogram.triggered.connect(self.show_amplitude_histogram); self.action_histogram.setEnabled(False); tools_menu.addAction(self.action_histogram)
